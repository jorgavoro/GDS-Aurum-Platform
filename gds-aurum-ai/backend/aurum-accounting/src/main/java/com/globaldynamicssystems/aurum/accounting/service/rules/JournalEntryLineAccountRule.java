...
@Component
public class JournalEntryLineAccountRule implements AccountingRuleEvaluator {

    private static final AccountingRule RULE = new AccountingRule(
            "RULE-006",
            "Line Must Have Account",
            "Every JournalEntryLine must reference a valid Account.",
    ...
    public AccountingRuleResult evaluate(AccountingRuleContext context) {
        ...
        if (line.getAccount() == null) {
            return fail("JournalEntryLine " + line.getId()
                    + " does not have an Account.", "account");
        }
        return pass();
    }
