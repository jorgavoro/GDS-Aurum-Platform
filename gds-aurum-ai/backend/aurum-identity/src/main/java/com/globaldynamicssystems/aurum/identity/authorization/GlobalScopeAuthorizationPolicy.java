...
@Component
public class GlobalScopeAuthorizationPolicy implements AuthorizationPolicy {

    @Override
    public boolean supports(AuthorizationRequest request, Permission permission) {
        return request.getScopeType() == PermissionScopeType.GLOBAL 
                && permission.getScopeType() == PermissionScopeType.GLOBAL;
    }
}